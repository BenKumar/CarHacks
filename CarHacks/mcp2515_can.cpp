...
static INT8U   m_nExtFlg;                                                  // Identifier Type
                                                                    // Extended (29 bit) or Standard (11 bit)
static INT32U  m_nID;                                                      // CAN ID
static INT8U   m_nDlc;                                                     // Data Length Code
static INT8U   m_nDta[MAX_CHAR_IN_MESSAGE];                              // Data array
static INT8U   m_nRtr;                                                     // Remote request flag
static INT8U   m_nfilhit;                                                  // The number of the filter that matched the message
static INT8U   MCPCS;                                                      // Chip Select pin number
static INT8U   mcpMode;                                                    // Mode to return to after configurations are performed.
...
static void mcp2515_reset(void);                                           // Soft Reset MCP2515

static INT8U mcp2515_readRegister(const INT8U address);                    // Read MCP2515 register

static void mcp2515_readRegisterS(const INT8U address,                     // Read MCP2515 successive registers
                           INT8U values[], 
                           const INT8U n);

static void mcp2515_setRegister(const INT8U address,                       // Set MCP2515 register
                         const INT8U value);

static void mcp2515_setRegisterS(const INT8U address,                      // Set MCP2515 successive registers
                          const INT8U values[],
                          const INT8U n);

static void mcp2515_initCANBuffers(void);

static void mcp2515_modifyRegister(const INT8U address,                    // Set specific bit(s) of a register
                            const INT8U mask,
                            const INT8U data);

static INT8U mcp2515_readStatus(void);                                     // Read MCP2515 Status
static INT8U mcp2515_setCANCTRL_Mode(const INT8U newmode);                 // Set mode
static INT8U mcp2515_configRate(const INT8U canSpeed,                      // Set baud rate
                         const INT8U canClock);
                         
static INT8U mcp2515_init(const INT8U canIDMode,                           // Initialize Controller
                   const INT8U canSpeed,
                   const INT8U canClock,
         const INT8U interruptRegister);
       
static void mcp2515_write_mf( const INT8U mcp_addr,                        // Write CAN Mask or Filter
                       const INT8U ext,
                       const INT32U id );
         
static void mcp2515_write_id( const INT8U mcp_addr,                        // Write CAN ID
                       const INT8U ext,
                       const INT32U id );

static void mcp2515_read_id( const INT8U mcp_addr,                         // Read CAN ID
                      INT8U* ext,
                      INT32U* id );

static void mcp2515_write_canMsg( const INT8U buffer_sidh_addr );          // Write CAN message
static void mcp2515_read_canMsg( const INT8U buffer_sidh_addr);            // Read CAN message
static INT8U mcp2515_getNextFreeTXBuf(INT8U *txbuf_n);                     // Find empty transmit buffer

/*********************************************************************************************************
 *  CAN operator function
 *********************************************************************************************************/

static INT8U mcp2515_setMsg(INT32U id, INT8U rtr, INT8U ext, INT8U len, INT8U *pData);        // Set message
static INT8U mcp2515_clearMsg();                                                   // Clear all message to zero
static INT8U mcp2515_readMsg();                                                    // Read message
static INT8U mcp2515_sendMsg();                                                    // Send message
...
*********************************************************************************************************/
static INT8U mcp2515_readRegister(const INT8U address)                                                                     
{
    INT8U ret;
...
*********************************************************************************************************/
static void mcp2515_readRegisterS(const INT8U address, INT8U values[], const INT8U n)
{
    INT8U i;
    SPI.beginTransaction(SPISettings(10000000, MSBFIRST, SPI_MODE0));
    MCP2515_SELECT();
...
*********************************************************************************************************/
static void mcp2515_setRegister(const INT8U address, const INT8U value)
{
    SPI.beginTransaction(SPISettings(10000000, MSBFIRST, SPI_MODE0));
    MCP2515_SELECT();
...
*********************************************************************************************************/
static void mcp2515_setRegisterS(const INT8U address, const INT8U values[], const INT8U n)
{
    INT8U i;
    SPI.beginTransaction(SPISettings(10000000, MSBFIRST, SPI_MODE0));
    MCP2515_SELECT();
...
*********************************************************************************************************/
static void mcp2515_modifyRegister(const INT8U address, const INT8U mask, const INT8U data)
{
    SPI.beginTransaction(SPISettings(10000000, MSBFIRST, SPI_MODE0));
    MCP2515_SELECT();
...
*********************************************************************************************************/
static INT8U mcp2515_readStatus(void)                             
{
    INT8U i;
    SPI.beginTransaction(SPISettings(10000000, MSBFIRST, SPI_MODE0));
    MCP2515_SELECT();
...
*********************************************************************************************************/
static INT8U mcp2515_setCANCTRL_Mode(const INT8U newmode)
{
    INT8U i;
...
*********************************************************************************************************/
static INT8U mcp2515_configRate(const INT8U canSpeed, const INT8U canClock)            
{
...
*********************************************************************************************************/
static INT8U mcp2515_init(const INT8U canIDMode, const INT8U canSpeed, const INT8U canClock, const INT8U interruptRegister)
{
  ...
  INT8U res;
    ...
    if ( res == MCP2515_OK ) {
        ...
        mcp2515_initCANBuffers();
                                                                        ...
                                                                        /* interrupt mode               */
        mcp2515_setRegister(MCP_CANINTE, interruptRegister);
...
*********************************************************************************************************/
static void mcp2515_write_id( const INT8U mcp_addr, const INT8U ext, const INT32U id )
{
    uint16_t canid;
    INT8U tbufdata[4];
...
*********************************************************************************************************/
static void mcp2515_write_mf( const INT8U mcp_addr, const INT8U ext, const INT32U id )
{
    uint16_t canid;
    INT8U tbufdata[4];
...
*********************************************************************************************************/
static void mcp2515_read_id( const INT8U mcp_addr, INT8U* ext, INT32U* id )
{
    INT8U tbufdata[4];
...
*********************************************************************************************************/
static void mcp2515_write_canMsg( const INT8U buffer_sidh_addr)
{
    INT8U mcp_addr;
...
*********************************************************************************************************/
static void mcp2515_read_canMsg( const INT8U buffer_sidh_addr)        /* read can msg                 */
{
    INT8U mcp_addr, ctrl;
...
*********************************************************************************************************/
static INT8U mcp2515_getNextFreeTXBuf(INT8U *txbuf_n)                 /* get Next free txbuf          */
{
    INT8U res, i, ctrlval;
...
*********************************************************************************************************/
INT8U mcp2515_begin(INT8U idmodeset, INT8U speedset, INT8U clockset, INT8U interruptset, INT8U _CS)
{
    INT8U res;
    ...
    SPI.begin();
    res = mcp2515_init(idmodeset, speedset, clockset, interruptset);
    if (res == MCP2515_OK)
        return CAN_OK;
...
** Descriptions:            Public function to set mask(s).
*********************************************************************************************************/
INT8U mcp2515_init_Mask(INT8U num, INT32U ulData)
{
    INT8U res = MCP2515_OK;
    INT8U ext = 0;
#if DEBUG_MODE
...
*********************************************************************************************************/
static INT8U mcp2515_setMsg(INT32U id, INT8U rtr, INT8U ext, INT8U len, INT8U *pData)
{
    int i = 0;
    m_nID     = id;
    m_nRtr    = rtr;
...
*********************************************************************************************************/
INT8U mcp2515_sendMsgBuf(INT32U id, INT8U ext, INT8U len, INT8U *buf)
{
    INT8U res;
...
*********************************************************************************************************/
INT8U mcp2515_readMsgBuf(INT32U *id, INT8U *ext, INT8U *len, INT8U buf[])
{
...
void mcp2515_resetInterrupts(void)
{
    mcp2515_setRegister(MCP_CANINTF, 0x00);
}

INT8U mcp2515_getInterrupts(void)
{
